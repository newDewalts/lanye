    ...
    public function add()
    {
    	if (IS_POST) {
    		...
    		if ($Model->create()) {
    			// 插入数据
    			if ($Model->add()) {
    				$this->success('添加成功！', U('Cooperation/index'));
    			} else {
    				$this->error('添加失败！');
    			}
    		} else {
    ...
    public function edit()
    {
    	if (IS_POST) {
    		...
    		if ($Model->create()) {
    			// 保存数据
    			if ($Model->save() !== false) {
    				$this->success('修改新闻成功！', I('post.forward_url', U('Cooperation/index')));
    			} else {
    				$this->error('修改新闻失败！');
    			}
    		} else {
    		...
    		$news = M('Cooperation')->find($id);
    		if (empty($news)) {
    			$this->error('合作机构不存在');
    		}
    		
    		$this->assign('news', $news);
    		$this->assign('forward_url', I('server.HTTP_REFERER', U('Cooperation/index')));
    		$this->display();
    	}
    }
