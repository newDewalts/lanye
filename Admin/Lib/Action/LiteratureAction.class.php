    ...
    public function add()
    {
    	if (IS_POST) {
    		...
    		if ($Model->create()) {
    			// 插入数据
    			if ($Model->add()) {
    				$this->success('添加文章成功！', U('Literature/index'));
    			} else {
    				$this->error('添加文章失败！');
    			}
    		} else {
    		...
    		// 文章分类
    		$this->assign('articleCatOptions', D('LiteratureCat') ->getFullOptions());
    		$this->display('edit');
		}
    }
    ...
    public function edit()
    {
    	if (IS_POST) {
    		...
    		if ($Model->create()) {
    			// 保存数据
    			if ($Model->save() !== false) {
    				$this->success('修改文章成功！', I('post.forward_url', U('Literature/index')));
    			} else {
    				$this->error('修改文章失败！');
    			}
    		} else {
    		...
    		$this->assign('forward_url', I('server.HTTP_REFERER', U('Literature/index')));
    		$this->display();
    	}
    }
