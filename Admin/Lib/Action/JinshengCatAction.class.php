 ...
 * @author 
 */
class JinshengCatAction extends CommonAction 
{
	// 文章分类列表
	public function index()
	{
		$this->assign('articleCatList', D('JinshengCat')->getFullOptions());
		$this->display();
	}
	...
	public function add()
	{
		...
		if (IS_POST) {
			...
			if ($Model->create()) {	// 创建数据
				...
				if ($Model->add()) { // 插入数据
					$this->success('添加文章分类成功！', U('JinshengCat/index'));
				} else {
					$this->error('添加文章分类失败！');
				}
			} else {
		...
		if (IS_POST) {
			...
			if ($Model->create()) {	// 创建数据
				...
				if ($Model->save() !== false) { // 插入数据
					$this->success('修改文章分类成功！', U('JinshengCat/index'));
				} else {
					$this->error('修改文章分类失败！');
				}
			} else {
